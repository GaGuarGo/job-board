...
<nav class="mb-8 flex justify-between text-lg font-medium">
    <ul class="flex space-x-2">
        <li><a href="{{route('jobs.index')}}"></a>Home</li>
    </ul>

    <ul class="flex flex-col space-y-2 justify-end items-end">
        @auth
            <li>
                <a href="{{route('my-job-applications.index')}}">{{auth()->user()->name ?? 'Anonymous'}}:
                    Applications</a>
            </li>
            <li>
                <a class="font-bold text-indigo-600" href="{{route('my-jobs.index')}}">My Jobs</a>
            </li>
            <li>
                <form action="{{route('auth.destroy')}}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button class="text-sm font-bold text-indigo-600 hover:underline">Logout</button>
                </form>
            </li>
        @else
            <li>
